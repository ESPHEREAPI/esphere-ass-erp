...
import { catchError, map, tap, retry, timeout, switchMap } from 'rxjs/operators';
...
export class AuthService {
  ...
  // Configuration  `${environment.apiUrl}/gateway-proxy/api/service-biometrie/auth`
  ...
  login(username: string, password: string): Observable<ApiResponse<UserSession>> {
    ...
    return this.http.post<ApiResponse<UserSession>>(`${this.apiUrl}/users/login`, loginRequest, { 
      headers,
    ...
    .pipe(
      ...
      tap(authResponse => {
        ...
        if (!this.isValidUserSession(authResponse.data)) {
          throw new Error('Session utilisateur invalide');
        }

        this.handleSuccessfulLogin(authResponse.data);
      })
    );
  }
